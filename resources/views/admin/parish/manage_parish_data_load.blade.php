    ...
    <table id="table_sort" class="table table-striped table-bordered">
        <thead>
            <tr>
              ...
              <th onclick="sortTable(1)">{{ __('Parish Name') }}
                <span class="sortTableIcon"></span>
              </th>
        ...
        <tbody>
          ...
          @foreach ($details_parish as $key => $value)
            <tr id="{{$value->id}}" @if($value->status==0) class="row_deactive" @endif>
              ...
              <a data-placement="top" data-toggle="tooltip" title="Edit" href="{{url('admin/parish/'.$value->id.'/edit')}}" class="btn btn-xs"><i class="fas fa-pencil-alt"></i></a>
              @if($value->status==1)
...
<div style="font-size: 1.3rem;"> {{$details_parish->links()}}</div>
    ...
    $(function() {
       $('tbody').sortable({
          ...
          var productOrder = $(this).sortable('toArray').toString();
             $.ajax({
               url:'{{ URL::asset('/parish/parish_tbl_sortable') }}',
                           type: 'POST',
